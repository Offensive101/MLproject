...
# Train the model
class da_rnn:
    def __init__(self, train_size_precentage,X,y, encoder_hidden_size = 64, decoder_hidden_size = 64, T = 10,
        ...
        self.train_size = int(train_size_precentage * self.X.shape[0])
    ...
    def Train(self, num_epochs = 10,perdiction_is_binary = False):
        logging.debug("**** in Dual LSTM train *****")
        iter_per_epoch = int(np.ceil(self.train_size * 1. / self.batch_size))
        logging.debug("Iterations per epoch: %3.3f ~ %d.", self.train_size * 1. / self.batch_size, iter_per_epoch)
        ...
        for i in range(num_epochs):
            ...
            #perm_idx = np.arange(self.train_size - self.T)
            j = 0

            logging.debug("Dual_Lstm_Attn: epoch num: ")
            logging.debug(str(i))

            while j < self.train_size :#for i, data in enumerate(self.train_loader):
                batch_idx = perm_idx[j:(j + self.batch_size)]
                x_curr_train = np.zeros((len(batch_idx), self.T - 1, self.X.shape[1]))
                y_history = np.zeros((len(batch_idx), self.T - 1))
                y_target = self.y[batch_idx + self.T]

                for k in range(len(batch_idx)):
                    x_curr_train[k, :, :] = self.X[batch_idx[k] : (batch_idx[k] + self.T - 1), :]
            ...
            if i % 10 == 0:
                logging.info("Epoch %d, loss: %3.3f.", i, self.epoch_losses[i])

            if i % 100 == 0:#TODO - can remove, takes running time
